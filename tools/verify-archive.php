...
$requiredShipped = [
    ...
    'docs/ownership.md',
    'docs/corpus.md',
    'resources/semantics/v1.json',
    'resources/ownership/v1.json',
    'resources/corpus/v1.json',
    'resources/corpus/v1.sha256',
...
foreach ($symbols as $symbol => $entry) {
    ...
    if (
        !is_string($symbol)
        || !str_starts_with($symbol, 'Kumwe\\CanonicalJson\\')
        || !is_string($file)
...
if (($composer['name'] ?? null) !== 'kumwe/canonical-json') {
    $errors[] = 'The archived composer.json does not name kumwe/canonical-json.';
}
...
if (($composer['autoload'] ?? null) !== ['psr-4' => ['Kumwe\\CanonicalJson\\' => 'src/']]) {
