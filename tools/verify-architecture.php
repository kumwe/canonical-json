...
$runtimeSelection = [
    ...
    'class_alias', 'class_exists', 'interface_exists', 'extension_loaded', 'function_exists',
];
$errors = [];
$files = [];
...
if (($composer['name'] ?? null) !== 'kumwe/canonical-json') {
    $errors[] = 'composer.json must name the package kumwe/canonical-json.';
}
...
if (($composer['autoload'] ?? null) !== ['psr-4' => [$namespaceRoot . '\\' => 'src/']]) {
    $errors[] = 'composer.json must autoload exactly the one canonical namespace Kumwe\\CanonicalJson\\ from src/.';
}
...
echo 'Architecture verified: ' . count($files)
    . " source files under Kumwe\\CanonicalJson, no host, driver or container coupling.\n";
