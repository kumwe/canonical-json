...
$workspace = rtrim(sys_get_temp_dir(), '/\\') . '/kumwe-canonical-json-consumer-' . bin2hex(random_bytes(6));
...
consumerRun(
    [
        ...
        '--format=zip',
        '--dir=' . $distDirectory,
        '--file=kumwe-canonical-json',
    ],
    $workspace,
);
$archive = $distDirectory . '/kumwe-canonical-json.zip';
if (!is_file($archive)) {
...
$consumerMetadata = [
    ...
    'license' => 'proprietary',
    'require' => ['kumwe/canonical-json' => $release],
    'repositories' => [
...
$installed = $consumer . '/vendor/kumwe/canonical-json';
...
foreach (array_keys($classmap) as $symbol) {
    if (
        ...
        || str_starts_with($symbol, 'Kumwe\\CanonicalJson\\Tests\\')
    ) {
