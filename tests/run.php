 ...
 *
 * Assertions come from Kumwe\CanonicalJson\Tests\TestCase. No framework, so the suite runs on any supported
...
spl_autoload_register(static function (string $class): void {
    $prefixes = [
        'Kumwe\\CanonicalJson\\Tests\\' => __DIR__ . '/',
        'Kumwe\\CanonicalJson\\' => dirname(__DIR__) . '/src/',
    ];
...
if ($files === []) {
    fwrite(STDERR, "CanonicalJson suite failed: no test case files were discovered.\n");
    exit(1);
}
...
foreach ($files as $file) {
    $class = 'Kumwe\\CanonicalJson\\Tests\\Case\\' . basename($file, '.php');
...
echo "\nCanonicalJson suite passed: {$totalTests} tests, {$totalAssertions} assertions.\n";
